...
data OutputEvent = Enter Did | Exit Did
...
fromDL dl nvf = case nvf of
  ...
  At i pos -> do
    ...
    evs <- geofences dl $ (\gid -> do
      -- updating regions
      reg <- fmap runIdentity $ view (region dl)
      cnd1 <- fmap (Set.member i . runIdentity) $ view (inside dl)
    ...
    traverse (\(gid, mev) -> case mev of
      ...
      Just (Enter did) -> void $ alarm dl $ do
        c <- fmap runIdentity $ view (current dl)
  ...
  Tick t -> do
    ...
    fmap runIdentity $ view' (alarm dl `composeM` wentOff dl)
  Destroy -> do
...
stateDL = GeoDL {
  ...
  , wentOff = \ra -> StateT (\s -> Identity (undefined, s))
